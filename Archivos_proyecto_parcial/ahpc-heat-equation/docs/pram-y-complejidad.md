...
8. [Número óptimo de procesos y escalabilidad débil](#8-número-óptimo-de-procesos-y-escalabilidad-débil)
...
La sección 6 reparte directamente $W_1$ entre $p$ procesadores (sumando el costo
de la reducción) para obtener $T_p$.
...
costo de comunicación con el modelo de latencia–ancho de banda visto en el curso
(Unidad 2): enviar un mensaje de $m$ palabras cuesta $t_s + t_w\,m$ ($t_s$ =
latencia/*startup*, $t_w$ = costo por palabra).

**Descomposición de dominio 2D.** `MPI_Dims_create` reparte la malla en una rejilla
lo más cuadrada posible, $\sqrt p \times \sqrt p$ — el caso $d=2$ de una malla
$d$-dimensional con $p$ nodos (Unidad 4.3, "Topologías"), donde el lado de cada
subdominio escala como $p^{1/d}$. Cada proceso posee un subdominio
...
## 8. Número óptimo de procesos y escalabilidad débil

**Número óptimo de procesos (para $n$ fijo).** Igual que en el ejemplo de KNN
visto en clase (Unidades 4.2/4.3) — donde el punto óptimo de procesos se obtiene
derivando $T(n,p)$ respecto a $p$ e igualando a cero —, minimizamos
$T_{\text{par}}(n,p)$ en $p$: el término de cómputo $\propto 1/p$ decrece y el de
...
**Escalabilidad débil.** ¿Cómo debe crecer $n$ con $p$ para mantener $E(n,p)$
(§7) aproximadamente constante? El término que domina al crecer $p$ es el de
ancho de banda, $\Theta(t_w\sqrt p/n)$; mantenerlo constante exige
$$\boxed{\,n=\Theta(\sqrt p) \quad\Longleftrightarrow\quad W_1=n^4=\Theta(p^2).\,}$$
Es una escalabilidad **moderada**: para conservar la eficiencia hay que aumentar
el problema aproximadamente con el cuadrado del número de procesos — típico de
los *stencils* de diferencias finitas.
...
**Modelo PRAM y trabajo–profundidad**
...
**Modelo de costos de comunicación y descomposición de dominio**

3. A. Grama, A. Gupta, G. Karypis, V. Kumar, *Introduction to Parallel Computing*,
   2.ª ed., Addison-Wesley, 2003. — Modelo $t_s,t_w$, costo de halos y el ejemplo
   del *stencil* 2D (base de §7); métricas de *speedup* y eficiencia.

**Esquema numérico, estabilidad CFL y convergencia (para $T=\Theta(n^2)$)**

4. *Notes: von Neumann Stability Analysis* (MIT 18.300). —
...
6. *An Efficient Explicit Scheme for Solving the 2D Heat Equation with Stability
...
7. `mpi4py` / MPI — comunicación punto a punto y colectivas (`Irecv`/`Isend`,
